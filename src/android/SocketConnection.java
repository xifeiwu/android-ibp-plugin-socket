...
public class SocketConnection {
    ...
    private int nPort = 0;
    private SocketPlugin mContext;
    public SocketConnection(SocketPlugin frame){
        mContext = frame;
        mAddress = getLocalIP();
    }
...
//        mFrame.myLog(name, msg);
    ...
    private void logToMainFrame(String name, String msg){
        logToConsole(name, msg);
    }

    private Thread ServerThread;
    public void startServerSocket(CallbackContext callbackContext, int port){
        PluginResult result;
        if(!isServerStarted){
            nPort = port;
            ...
            ServerThread.start();
            result = new PluginResult(PluginResult.Status.OK, "startServerSocket: success.");
//            result.setKeepCallback(true);
        }else{
            this.logToMainFrame("startServerSocket", "ServerSocket Has Started Already.");
            result = new PluginResult(PluginResult.Status.ERROR, "startServerSocket: ServerSocket Has Started.");
        }
        callbackContext.sendPluginResult(result);
    }
    public void stopServerSocket(CallbackContext callbackContext){
        PluginResult result;
        if((null != ServerThread) && (!mServerSocket.isClosed())){
            ...
            try {
                ...
                result = new PluginResult(PluginResult.Status.OK, "stopServerSocket: success.");
            } catch (IOException ioe) {
                logToMainFrame("LOG", "Error when stop ServerSocket, As Below:");
                ioe.printStackTrace();
                result = new PluginResult(PluginResult.Status.ERROR, "stopServerSocket: Error when stop ServerSocket.");
            }
        } else {
    ...
    private Runnable ServerRunnable = new Runnable() {
        ...
        public void run() {
            try {
                ...
                logToMainFrame("LOG", "ServerSocket " + mAddress + ":" + nPort + " started");
                mServerSocket = new ServerSocket(nPort);
                mServerSocket.setReuseAddress(true);
                while (!Thread.currentThread().isInterrupted() && isServerStarted) {
                    ...
                    if(mMsgObj != null){
                        ...
                        mContext.processMsgObj(mMsgObj);
                    }
                    remoteSocket.close();
                }
            } catch (IOException e) {
...
//            input.close();
    ...
    private void replyMessage(Socket socket, JSONObject mMsgObj) throws JSONException{
